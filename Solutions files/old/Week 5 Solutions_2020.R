...
urchin$TREAT <- as.factor(urchin$TREAT)
urchin$PATCH <- as.factor(urchin$PATCH)

table(urchin$TREAT, urchin$PATCH)


# plot
ggplot(urchin, aes(TREAT, ALGAE)) +
geom_boxplot()

ggplot(urchin, aes(PATCH, ALGAE)) +
  geom_boxplot()

# examine treatment effect using a nested ANOVA

full <- aov(ALGAE ~ TREAT + Error(TREAT/PATCH), data=urchin)
summary(full)

# STEP ONE - the food effect comparison
# You need the F statistic for the food effect so divide the mean sq on the top level 
# (TREAT) by the mean sq for the level below (TREAT : PATCH)
...
1 - pf(8.63902, 3, 12) # gives  a Treatment effect of (0.002514587 or 0.0025 if we round it)

# STEP TWO - the patch effect. Divide the TREATMENT/PATCH residuals by the error within residuals
1770/298.6 # this is 5.927662

# Compute the p value
1 - pf(5.927662, 6, 16) # 0.002031247)

PatchEffect <- aov(ALGAE ~ TREAT + Error(TREAT : PATCH), data = urchin)
