...
format:
  html:
    ...
    theme: /Users/jonsadler/Documents/GitHub/Common Resources/custom-fonts.scss
---
...
2.  从我的 GitHub 代码库下载人口普查数据。
3.  使用 RStudio 将数据加载到 R 中。
4.  在 R 中查看原始数据并进行修改。
5.  在 R 中对数据进行子集化。
6.  在 R 中合并数据。
...
我们将使用的统计和编程语言是 R。它（与 Python 一起）已成为处理地理和环境数据的行业标准。与 Excel 或 SPSS 等“现成”软件不同，用户必须输入命令才能执行任务，例如加载数据集或执行计算。这种方法的优势在于它会创建一个文档或脚本，记录您的所有工作，供所有人查看。它还提供可重复使用的文档，使您可以直接重复执行产生相同结果的任务！
...
R 语言可以从 https://www.r-project.org/ 下载。如果您的电脑上还没有 R 语言，请查看 CANVAS 网站上的会前模块以获取指导。该模块位于我们大学的计算机集群上，可通过 AppsAnywhere 访问。所有电脑的桌面上都有 R 语言的快捷方式。虽然可以直接在 R 语言上进行分析，但我们将通过 Rstudio 运行它，因为它提供了用户友好的图形用户界面。下载 R 语言后，您可以从 https://www.rstudio.com/ 免费获取 Rstudio。
...
![](images/RStudio_Screen_Grab.png) 
**Fig. 1: RStudio screen windows**
...
setwd("~/Documents/GitHub/Teaching/LM_25556Environmental_Analysis/Codefiles")
```
...
![](images/Object_exampleRStudio.png) **Fig. 2: The environment window function.**
...
Most of the following derives from Zuur et al. 2009 Beginner's Guide to R. First Steps - typing in a small dataset.
...
![](images/Set_path_directory.png) **Fig. 3: Copying the path information.**
...
kable(deer_data, caption = "Deer Measurement Data")
...
这些数据是 Zuur 等人（2009 年）使用的数据文件的一小部分（完整数据请见 deer.txt）。
