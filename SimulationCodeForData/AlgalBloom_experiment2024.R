...
nutrients<- rep(c("meso","eutro"),2, each=20)
AllData<-data.frame(reps,SR,temp,nutrients)

ANOVAresults<-aov(SR~temp*nutrients, AllData)

summary(ANOVAresults)

model.tables(ANOVAresults,"means")

interaction.plot(temp,nutrients,SR) 
boxplot(SR~nutrients,data=AllData)
boxplot(SR~temp,data=AllData)
